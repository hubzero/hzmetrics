...
2. Now check the database to see if the data got in.
	...
	select * from webhits;
	select * from web;
	select count(*) from bot_useragents;
        ```

	There should be data in each table.

	As the apache user, test that xlogfix_prep ran:

	```
        $ sudo su - apache 
	$ ls /etc/hubzero-metrics/access.cfg
        ```

	you should see a file owned by root:apache listed.

	Test that xlogimport_tool_and_reg_user_data ran:

	`mysql> select * from toolstart;`

        If any tools were run, there should be data with some positive values for cputime (the last column).
...
3. Create a link from your menu to the usage component in Joomla. There may not be any data in the Overview view.
	...
	That is because the __process_usage_metrics_summary.sh script only runs on the first of the month. 
        Run it as the apache user, and you'll see data.
