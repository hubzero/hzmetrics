...
$sql = 'SELECT DISTINCT(INET_ATON(ip)) AS n_ip, COUNT(*) AS hits FROM '.$database.'.'.$table.' WHERE '.$datetime_col.'> "'.$queryDate->format('Y-m-d').'" AND (ipcountry = "" OR ipcountry IS NULL) GROUP BY n_ip ORDER by hits desc;';
