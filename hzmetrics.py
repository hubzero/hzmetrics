...
#
# There is a 4th historical shape — the 2021-05..2022-02 cmsauth logs
# dropped the HH:MM:SS component entirely (logger bug during that
# window).  We do NOT recognize that format here: synthesizing a fake
# 00:00:00 time would let the data import, but every line in a given
# day shares the same synthetic timestamp, which causes the
# reconstruct's MIN/MAX(id)-per-line-key bounds to collapse across
# files for that period (200+ ledger overlaps + 100+ survivor-
# invariant breaks on geodynamics).  Lines from that 10-month window
# fall through to the unrecognized counter, which is the desired
# behavior: the asymmetric-coverage audit will flag the affected
# months and the operator can decide whether to live with the gap.
